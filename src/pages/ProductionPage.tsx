...
import { CardGrid, StatCard, PrimaryCard } from '../components/ui/Card';
...
export const ProductionPage: React.FC = () => {
  ...
  const { predictedProduction, shortfall, shortfallPct, confidencePct } =
    forecastService.calculateForecast(productionFactors);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <CardGrid columns={4}>
        <StatCard
          title="Target Tonnage"
          value={`${target.toLocaleString()} MT`}
          subtext="30-Day Benchmark"
          icon={TrendingUp}
        />
        <StatCard
          title="Predicted AI Output"
          value={`${predictedProduction.toLocaleString()} MT`}
          subtext={isAhead ? '+ Ahead of Target' : `Variance: ${shortfallPct}%`}
          icon={Activity}
        />
        <StatCard
          title="Shortfall Gap"
          value={`${shortfall.toLocaleString()} MT`}
          subtext="Mitigation Action Required"
          icon={AlertTriangle}
        />
        <StatCard
          title="Forecast Confidence"
          value={`${confidencePct}%`}
          subtext="Statistical Fidelity"
          icon={Cpu}
        />
      </CardGrid>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <PrimaryCard title="Production Forecast vs Target" subtitle="HISTORICAL BASELINE VS AI REGRESSION">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#1D2517]">
              <div>
                <p className="text-xs text-[#71825B] font-mono">
                  Multi-factor neural timeline projection
                </p>
              </div>
        ...
        <PrimaryCard title="Operational Factors" icon={Sliders}>
          <div className="space-y-4">
            ...
            <div className="space-y-1.5 text-xs font-mono">
              <div className="flex justify-between">
                ...
                <span className="text-[#A4B18A] font-bold">
                  {productionFactors.equipmentAvailability}%
                </span>
              </div>
              <input
                type="range"
                  ...
                  <button
                    ...
                    className={`py-1.5 rounded-lg border text-xs font-bold transition cursor-pointer ${
                      productionFactors.rainfallRiskLevel === lvl
                        ...
                        : 'bg-[#0B0E09] border-[#252E1D] text-[#71825B] hover:text-[#F1F2E9]'
                    }`}
                  >
                    {lvl}
                  </button>
            ...
            <div className="space-y-1.5 text-xs font-mono">
              <div className="flex justify-between">
                ...
                <span className="text-[#A4B18A] font-bold">
                  {productionFactors.blastingDelayDays} Days
                </span>
              </div>
              <input
                type="range"
            ...
            <div className="space-y-1.5 text-xs font-mono">
              <div className="flex justify-between">
                ...
                <span className="text-[#A4B18A] font-bold">
                  {productionFactors.transportAvailability}%
                </span>
              </div>
              <input
                type="range"
            ...
            <div className="space-y-1.5 text-xs font-mono">
              <div className="flex justify-between">
                ...
                <span className="text-[#A4B18A] font-bold">
                  {productionFactors.oreAvailability}%
                </span>
              </div>
              <input
                type="range"
